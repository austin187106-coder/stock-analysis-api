...
from modules.data_module import StockDataService, TWSEDataFetcher, YFinanceDataFetcher
...
def _build_data_service() -> StockDataService:
    """依 config.py 的 data_source 設定選擇資料來源"""
    if settings.data_source == "yfinance":
        return StockDataService(fetcher=YFinanceDataFetcher())
    return StockDataService(fetcher=TWSEDataFetcher())
...
def analyze_triple_screen(
    ...
    except Exception as exc:  # noqa: BLE001 — 外部資料來源的例外型態不可控，統一轉為明確訊息
        raise HTTPException(
            status_code=503,
            detail=f"資料來源暫時無法取得資料，請稍後再試（{type(exc).__name__}）",
        ) from exc
